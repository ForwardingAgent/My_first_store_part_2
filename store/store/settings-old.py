...
env_file = os.path.join(BASE_DIR / '.env')  # определяем путь до файла .env
environ.Env.read_env(env_file)  # показываем по какому пути читать переменные
...
if DEBUG:
    ...
    DOMAIN_NAME = 'http://my-online-store.ru'  # для деплоя
...
DATABASES = {
    'default': {
        ...
        'HOST': env('DB_HOST'),  # Наименование контейнера для базы данных в Docker Compose
        'PORT': env('DB_PORT'),
...
if DEBUG:
    ...
    #  STATICFILES_DIRS и STATIC_ROOT не могут быть вместе
