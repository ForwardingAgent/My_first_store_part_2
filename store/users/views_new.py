...
class UserRegistrationView(CreateView):  # 7.6
    ...
    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:  # get_context_data можно использовать т.к. наследуется от ContextMixin
        context = super(UserRegistrationForm).get_context_data(**kwargs)  # cоздает словарь | сначала через super вызываем родительский метод, чтобы он выполнился, и ниже добавляем наши ключи в словарь (т.е. переопределили род.метод)
        context['title'] = 'Store - Регистрация'  # дополняем словарь своими данными
        return context


class UserProfileView(UpdateView):  # 7.6
    model = User
    ...
    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:  # get_context_data можно использовать т.к. наследуется от ContextMixin
        ...
        context['title'] = 'Store - Профиль'  # дополняем словарь своими данными
